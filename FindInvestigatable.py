...
data = spark.read.option("inferSchema", True).option("header", True).csv("./Data/FINALDATACONVERTEDNOMINALSNOZIP.csv")
#data.printSchema()
...
evaluator = MulticlassClassificationEvaluator(labelCol="ReportedFraud",
...
factor = 1
successRate = 1
expectedValueOfInvestigation=udf(lambda v,v2:float(v[1]*v2*0.88 * factor * successRate),FloatType())
predictions.select("CustomerID", "ReportedFraud", "prediction", "probability", "AmountOfTotalClaim", expectedValueOfInvestigation("probability", "AmountOfTotalClaim")).orderBy(expectedValueOfInvestigation("probability", "AmountOfTotalClaim"), ascending=False).show(10, truncate = False)
